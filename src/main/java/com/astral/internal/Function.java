...
public class Function {
    ...
    public static void invoicePDF(String invID) {
        ...
        try (Document document = new Document()) {
            ...
            para.setAlignment(Element.ALIGN_CENTER);
            document.add(para);
            para = new Paragraph("Contact Number : " + SQLite.getConfigValue("Country Code") + "-" +
                SQLite.getConfigValue("Contact Number").replaceFirst("(\\d{5})(\\d+)", "$1-$2") + "    Email : " + SQLite.getConfigValue("Email Address"));
            para.setAlignment(Element.ALIGN_CENTER);
